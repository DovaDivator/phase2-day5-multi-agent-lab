"""Benchmark report rendering in Vietnamese with Failure Mode analysis."""

from multi_agent_research_lab.core.schemas import BenchmarkMetrics


def render_markdown_report(metrics: list[BenchmarkMetrics]) -> str:
    """Render benchmark metrics to markdown in Vietnamese."""

    lines = [
        "# 📊 Báo cáo Benchmark toàn diện: Hệ thống Nghiên cứu Multi-Agent",
        "",
        "## 1. Tóm tắt điều hành (Executive Summary)",
        "Báo cáo này đánh giá hiệu suất, độ chính xác và tính hiệu quả về chi phí của "
        "**Hệ thống Nghiên cứu Multi-Agent** so với **Single-Agent Baseline (Mô hình cơ sở đơn tác tử)**.",
        "Mục tiêu là xác định liệu sự phức tạp tăng thêm của mô hình phối hợp (Supervisor-Worker-Critic) "
        "có mang lại cải thiện rõ rệt về chất lượng nghiên cứu hay không.",
        "",
    ...
    lines.extend([
        ...
        "Trong **Single-Agent Baseline (Đơn tác tử)**, LLM phụ thuộc hoàn toàn vào kiến thức nội tại "
        "hoặc một bước truy xuất duy nhất. Điều này thường dẫn đến phân tích bề mặt. "
        "**Hệ thống Multi-Agent (Đa tác tử)** tận dụng vòng lặp *Supervisor-Analyst*. Nếu phát hiện lỗ hổng, "
        "Supervisor sẽ kích hoạt thêm bước *Researcher* để đảm bảo dữ liệu được thu thập đầy đủ trước khi tổng hợp.",
        "",
        "### B. Đánh đổi kiến trúc (Architectural Trade-offs)",
        "1. **Chi phí suy luận (Cost of Reasoning)**: Hệ thống Multi-Agent thực hiện nhiều lượt gọi LLM hơn đáng kể. "
        "Đối với nghiên cứu phức tạp, đây là 'chi phí của chất lượng'.",
        "2. **Chuyên môn hóa vai trò (Role Specialization)**: Bằng cách tách biệt các vai trò *Analyst (Phân tích)*, "
        "*Writer (Người viết)*, và *Critic (Người phản biện)*, chúng ta giảm thiểu hiện tượng 'nhầm lẫn vai trò' "
        "và đảm bảo báo cáo cuối cùng đạt tiêu chuẩn cao nhất.",
        "",
        ...
        "Nên dùng cho các tác vụ **nghiên cứu chuyên sâu (Deep Research)** hoặc lập kế hoạch phức tạp. "
        "Lý do: Khả năng tự sửa lỗi và kiểm định chéo giữa các agents giúp đảm bảo độ tin cậy của thông tin.",
        "",
        "**Q2: Case nào không nên dùng multi-agent (đa tác tử)? Vì sao?**",
        "Không nên dùng cho các tác vụ **hỏi đáp nhanh (Q&A)** hoặc tóm tắt đơn giản. "
        "Lý do: Chi phí và độ trễ cao không tương xứng với giá trị nhận lại trong các tình huống đơn giản.",
        "",
